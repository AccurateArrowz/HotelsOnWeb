...
const RoomTypesManagement = ({ hotelId }: RoomTypesManagementProps) => {
  const { data: roomTypes, isLoading, error, refetch } = useGetRoomTypesByHotelQuery(hotelId);
  ...
  const handleOpenModal = (roomType: any = null) => {
    if (roomType) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      adults: parseInt(String(formData.adults)),
      children: parseInt(String(formData.children)),
    };
  ...
  const handleDelete = async (roomType: any) => {
  ...
  const handleToggleActive = async (roomType: any) => {
    try {
      await updateRoomType({
        hotelId,
        roomTypeId: roomType.id,
    ...
    return (
      <div className="room-types-management">
        <div className="error-state">
          <p>Error loading room types: {(error as any).data?.message || 'Unknown error'}</p>
