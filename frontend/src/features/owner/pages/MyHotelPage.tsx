...
import InviteStaff from '@features/owner/components/InviteStaff';
...
interface NavItem {
  id: 'overview' | 'bookings' | 'rooms' | 'room-types' | 'staff' | 'analytics';
  icon: LucideIcon;
  label: string;
}
...
const OverviewSection = (): ReactNode => (
...
const BookingsSection = (): ReactNode => (
...
const PlaceholderSection = ({ title }: PlaceholderSectionProps): ReactNode => (
...
const contentComponents: Record<string, React.ComponentType<ContentComponentProps>> = {
  ...
  bookings: BookingsSection as any,
...
const MyHotelPage = () => {
  const [activeTab, setActiveTab] = useState<'overview' | 'bookings' | 'rooms' | 'room-types' | 'staff' | 'analytics'>('overview');
  ...
  const handleNavClick = (tabId: 'overview' | 'bookings' | 'rooms' | 'room-types' | 'staff' | 'analytics') => {
    setActiveTab(tabId);
    setSidebarOpen(false);
  };

  const handleHotelChange = (newHotel: Hotel) => {
    ...
    return (
      <div className="my-hotel-page error">
        <div className="error-message">
          ...
          <p>{(hotelsError as any).data?.message || 'An error occurred while fetching your hotel data.'}</p>
