...
import { User } from '@hotelsonweb/shared';
import { useAuth, RequireAuth, RequireRole, LoginForm } from '@features/auth';
import { Modal } from '@shared/components';
import Spinner from '@shared/components/Spinner';
import { uploadFiles } from '@services/mediaUpload';
import { useCreateHotelRequestMutation } from '@features/owner/api';
...
const ListYourProperty = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const imageUrls = uploads.map((u: any) => u.url).filter(Boolean);
  ...
  if (user && (user as User).role !== 'owner') {
    return (
      <div style={{ padding: '2rem', textAlign: 'center' }}>
        <h1>List Your Property</h1>
