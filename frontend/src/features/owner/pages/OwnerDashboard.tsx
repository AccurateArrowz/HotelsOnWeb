...
import { Room } from '@hotelsonweb/shared';
import {
  LayoutDashboard,
  CalendarDays,
  BedDouble,
...
interface Booking {
  ...
  checkOut: string;
  amount: string;
  status: 'active' | 'upcoming' | 'checkout' | 'completed';
}
...
interface NavItem {
  id: 'overview' | 'bookings' | 'rooms' | 'analytics';
  Icon: LucideIcon;
  label: string;
}
...
const bookingStatusStyle: Record<string, BookingStatusStyle> = {
...
const KPICard = ({ Icon, label, value, sub, trend, trendUp }: KPICardProps): ReactNode => (
...
export default function OwnerDashboard() {
  const [activeTab, setActiveTab] = useState<'overview' | 'bookings' | 'rooms' | 'analytics'>("overview");
              ...
              <div style={{ background: "#fff", borderRadius: 16, padding: 20, border: "1px solid #f1f5f9" }}>
                ...
                <ResponsiveContainer width="100%" height={190}>
                  <AreaChart data={revenueData}>
                    ...
                    <Tooltip formatter={(val: any, name: string) => name === "revenue" ? [`$${val.toLocaleString()}`, "Revenue"] : [`${val}%`, "Occupancy"]} />
            ...
            <div style={{ background: "#fff", borderRadius: 16, padding: 20, border: "1px solid #f1f5f9" }}>
              ...
              <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
                {Object.entries(statusConfig).map(([key, cfg]) => {
                  const count = rooms.filter(r => r.status === key as any).length;
                  return (
